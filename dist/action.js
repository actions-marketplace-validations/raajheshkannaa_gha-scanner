...
// src/lib/utils/parse-repo.ts
var REPO_REGEX = /^[a-zA-Z0-9]([a-zA-Z0-9._-]*[a-zA-Z0-9])?\/[a-zA-Z0-9]([a-zA-Z0-9._-]*[a-zA-Z0-9])?$/;
...
var MAX_ALIAS_COUNT = 10;
function parseWorkflowYaml(content, filename) {
  if (content.length > MAX_FILE_SIZE) {
    return {
      parsed: null,
      error: `${filename}: File exceeds 1MB size limit (${(content.length / 1e6).toFixed(1)}MB)`
    };
  }
  try {
...
// src/lib/github/fetch-workflows.ts
...
var GitHubNotFoundError = class extends GitHubError {
  constructor(owner, repo) {
    super(`Repository ${owner}/${repo} not found or is not accessible.`);
    this.name = "GitHubNotFoundError";
  }
};
var GitHubRateLimitError = class extends GitHubError {
  constructor(resetEpoch) {
    const resetAt = new Date(resetEpoch * 1e3);
...
async function githubFetch(path, accept = "application/vnd.github+json") {
  ...
  if (response.status === 403 && rateLimit.remaining !== -1 && rateLimit.remaining === 0) {
...
async function discoverWorkflowPaths(owner, repo, branch) {
  const { body, rateLimit } = await githubFetch(
...
async function fetchRepoContext(owner, repo) {
  ...
  function trackRate(rl) {
    if (rl.remaining !== -1) {
      latestRateLimit = rl;
    }
  }
  function guardRateLimit() {
    if (latestRateLimit.remaining !== -1 && latestRateLimit.remaining < RATE_LIMIT_FLOOR) {
...
// src/lib/scanner/data/mutable-refs.ts
var MUTABLE_REFS = [
  "main",
  "master",
  "latest",
...
// src/lib/scanner/data/known-vulnerable-actions.ts
var KNOWN_VULNERABLE_ACTIONS = [
  {
    ...
    fixedVersion: "46.0.1",
    description: "Supply chain compromise exposing CI/CD secrets in workflow logs for 23,000+ repos"
  },
  {
  ...
  },
  {
    ...
    description: "TeamPCP poisoned 75 of 76 version tags after botched credential rotation. Memory dumping stealer targeting Runner.Worker process."
  },
  {
    action: "aquasecurity/setup-trivy",
    affectedVersions: "all tags (compromised Mar 2026)",
    disclosedDate: "2026-03-12",
    description: "Compromised alongside trivy-action in the same supply chain attack"
  },
  {
    ...
    fixedVersion: "1.0.2",
    description: "Bash uploader modified to export environment variables and secrets"
  }
];

// src/lib/scanner/checks/supply-chain.ts
...
function parseMarketplaceAction(uses) {
...
function extractUsesSteps(workflow) {
  const results = [];
...
var supplyChainChecks = [
  {
    ...
    description: "Detects marketplace actions not pinned to a full-length commit SHA, making them vulnerable to tag mutation attacks.",
  ...
  },
  {
    ...
    description: "Detects actions pinned to branch names like main, master, or other refs that change with every commit.",
    category: "supply-chain",
    severity: "high",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        for (const step of steps) {
          ...
          const severity = isSameOrg ? "medium" : "high";
          const sameOrgNote = isSameOrg ? " This is a same-org action, so the risk is lower (you control the source), but it should still be pinned to a SHA for auditability and supply chain hygiene." : "";
          findings.push({
...
Resolve the SHA: \`gh api repos/${parsed.action}/git/ref/heads/${parsed.ref} --jq .object.sha\``,
  ...
  },
  {
    ...
    description: "Detects usage of GitHub Actions with known CVEs or confirmed supply chain compromises.",
    category: "supply-chain",
    severity: "critical",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        for (const step of steps) {
          ...
          findings.push({
            ...
            remediation: vuln.fixedVersion ? `Update to the fixed version and pin by SHA:
...
Verify the fix version SHA: \`gh api repos/${parsed.action}/git/ref/tags/v${vuln.fixedVersion} --jq .object.sha\`` : `No fixed version is available. Consider removing this action entirely and replacing it with an alternative, or forking a known-good commit:
...
\`\`\``,
  ...
  },
  {
    ...
    description: "Detects docker:// action references using mutable tags like :latest or no tag at all.",
    category: "supply-chain",
    severity: "high",
    run(context) {
      const findings = [];
...
// src/lib/scanner/data/dangerous-contexts.ts
var DANGEROUS_CONTEXTS = [
...
// src/lib/scanner/checks/injection.ts
var EVENT_EXPRESSION_PATTERN = /\$\{\{[^}]*github\.event\.[^}]*\}\}/g;
function walkRunSteps(workflow) {
  const results = [];
  if (!workflow.parsed) return results;
  const jobs = workflow.parsed.jobs;
  if (!jobs || typeof jobs !== "object") return results;
  for (const [jobId, jobDef] of Object.entries(jobs)) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      results.push({
        ...
        jobId,
        stepIndex: i,
        stepName: typeof step.name === "string" ? step.name : void 0,
        file: workflow.path,
        content: workflow.content
      });
    }
...
function getWorkflowDispatchInputs(parsed) {
  const on = parsed.on ?? parsed.true;
  if (!on || typeof on !== "object") return null;
  const onObj = on;
  const dispatch = onObj.workflow_dispatch;
...
var injectionChecks = [
  {
    ...
    description: "Detects ${{ github.event.* }} expressions used directly inside run: blocks. While not all event properties are attacker-controlled, using them inline is a risky pattern.",
    category: "injection",
    severity: "low",
    run(context) {
      const findings = [];
      const SAFE_EVENT_PROPS = [
        ...
        "github.event.inputs."
        // handled by dispatch-input check
      ];
      for (const workflow of context.workflows) {
        for (const step of walkRunSteps(workflow)) {
          ...
          for (const match of matches) {
            ...
            const stepLabel = step.stepName ? `step "${step.stepName}"` : `step ${step.stepIndex + 1}`;
            findings.push({
...
\`\`\``,
  ...
  },
  {
    ...
    description: "Detects specific GitHub context variables known to be attacker-controlled when used in run: blocks.",
    category: "injection",
    severity: "critical",
    run(context) {
      const findings = [];
...
\`\`\``,
  ...
  },
  {
    ...
    description: "Detects workflow_dispatch input values used directly in run: blocks without sanitization. Exploitation requires write access to the repository.",
    category: "injection",
    severity: "medium",
    run(context) {
      const findings = [];
...
// src/lib/scanner/checks/dangerous-triggers.ts
function getTriggers(parsed) {
  const on = parsed["on"] ?? parsed["true"];
  if (!on) return [];
...
var dangerousTriggersChecks = [
  {
    id: "triggers/prt-checkout",
    name: "pull_request_target with head checkout",
    description: "Detects pull_request_target workflows that check out the PR head ref, allowing attacker-controlled code to run with write permissions.",
      ...
      for (const workflow of context.workflows) {
        ...
        for (const job of jobs) {
          for (const step of getSteps(job)) {
            ...
            if (ref.includes("github.event.pull_request.head.ref") || ref.includes("github.event.pull_request.head.sha")) {
              ...
              findings.push({
                ...
                description: `Workflow "${workflow.name}" uses pull_request_target and checks out the PR head via \`${ref}\`. This executes attacker-controlled code with the permissions of the base branch, including write access and secrets.`,
                risk: "An attacker can open a PR with a malicious workflow modification that runs with full repo write permissions and access to secrets. This was the attack vector in the GitHub Actions pwn requests research (2021) and has been exploited against major open-source projects.",
                remediation: "Avoid checking out PR head code in pull_request_target workflows. If you must, use a two-workflow pattern:\n\n```yaml\n# Workflow 1: pull_request_target (trusted code only)\non: pull_request_target\njobs:\n  build:\n    steps:\n      - uses: actions/checkout@v4  # checks out base, not PR head\n```\n\nIf PR code must be analyzed, run it in an isolated job without secrets access, or use `workflow_run` with artifact passing.",
                file: workflow.path,
  ...
  },
  {
    ...
    description: "Detects pull_request_target workflows that reference secrets, which may be exposed to forked PR authors.",
    category: "dangerous-triggers",
    severity: "high",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        while ((match = secretsPattern.exec(workflow.content)) !== null) {
          ...
          findings.push({
            ...
            description: `Workflow "${workflow.name}" uses pull_request_target and references secrets. Combined with any code execution from the PR head, this can leak repository secrets to external contributors.`,
            risk: "Secrets in pull_request_target workflows are available even for PRs from forks. If any step executes PR-controlled code (even indirectly through build scripts or Makefiles), secrets can be exfiltrated. This pattern was exploited in the 2021 attack against the GitHub Actions ecosystem.",
            remediation: "Remove secrets from pull_request_target workflows. Use the `pull_request` event instead (which restricts secrets from forks), or:\n\n```yaml\n# Pass only the minimum needed via environment\non: pull_request_target\njobs:\n  label:\n    runs-on: ubuntu-latest\n    # Only use secrets in steps that do NOT run PR code\n    steps:\n      - uses: actions/labeler@v5  # trusted action, no PR code execution\n```",
            file: workflow.path,
            line: findLineNumber(workflow.content, evidence),
            evidence
          });
  ...
  },
  {
    ...
    description: "Detects workflow_run triggers that download artifacts, which can be a vector for artifact poisoning attacks.",
    category: "dangerous-triggers",
    severity: "medium",
    run(context) {
      const findings = [];
        ...
        for (const job of jobs) {
          for (const step of getSteps(job)) {
            ...
            findings.push({
              ...
              description: `Workflow "${workflow.name}" uses workflow_run trigger and downloads artifacts. Artifacts from the triggering workflow (often a PR build) may contain attacker-controlled content.`,
              risk: "Artifacts produced by pull_request workflows can contain malicious payloads. If the workflow_run handler extracts and executes artifact contents (e.g., running scripts, importing code, or using paths from artifact data), an attacker can achieve code execution in a privileged context. This is a known artifact poisoning vector.",
              remediation: 'Treat all downloaded artifacts as untrusted input:\n\n```yaml\non:\n  workflow_run:\n    workflows: ["Build"]\n    types: [completed]\njobs:\n  deploy:\n    steps:\n      - uses: actions/download-artifact@v4\n      # Validate artifact contents before use\n      - run: |\n          # Verify checksums, validate file types\n          # Never execute downloaded scripts directly\n          sha256sum -c checksums.txt\n```\n\nConsider using OIDC or signed artifacts instead of relying on artifact download for sensitive deployments.',
              file: workflow.path,
              line: findLineNumber(workflow.content, evidence),
              evidence
            });
...
// src/lib/scanner/checks/permissions.ts
function getJobs2(parsed) {
  const jobs = parsed["jobs"];
  if (!jobs || typeof jobs !== "object") return [];
  return Object.entries(jobs).filter(
    (entry) => typeof entry[1] === "object" && entry[1] !== null
  );
}
function countWriteScopes(permissions) {
  return Object.values(permissions).filter((v) => v === "write").length;
}
var permissionsChecks = [
  {
    id: "permissions/missing-block",
    name: "Missing top-level permissions",
    description: "Checks whether workflows declare a top-level permissions block to restrict the default GITHUB_TOKEN scope.",
      ...
      for (const workflow of context.workflows) {
        ...
        if (!("permissions" in workflow.parsed)) {
          findings.push({
            ...
            description: `Workflow "${workflow.name}" does not declare a top-level \`permissions:\` block. Without explicit permissions, the GITHUB_TOKEN receives the default permissions configured in repository settings (often read-write for all scopes).`,
            risk: "The default GITHUB_TOKEN permissions may grant write access to contents, packages, issues, and more. If a workflow is compromised (e.g., through injection or a supply chain attack), the broad token scope increases blast radius. GitHub recommends setting the default to read-only and explicitly granting write scopes per workflow.",
  ...
  },
  {
    ...
    description: "Detects workflows that grant write-all or many write scopes at the top level.",
    category: "permissions",
    severity: "high",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        if (permissions === "write-all") {
          ...
          const writeCount = countWriteScopes(permissions);
          if (writeCount >= 3) {
            isOverlyBroad = true;
            const writeScopes = Object.entries(permissions).filter(([, v]) => v === "write").map(([k]) => k);
        ...
        if (isOverlyBroad) {
          findings.push({
            ...
            description: `Workflow "${workflow.name}" grants excessive write permissions to the GITHUB_TOKEN. ${evidence}.`,
            risk: "Broad token permissions increase the impact of any workflow compromise. An attacker exploiting an injection or compromised action gains write access to multiple repository resources. The principle of least privilege should be applied to CI/CD tokens.",
            remediation: "Restrict permissions to only what the workflow needs:\n\n```yaml\npermissions:\n  contents: read\n  pull-requests: write  # only if needed\n```\n\nSplit workflows that need different permissions into separate files, each with minimal scopes.",
            file: workflow.path,
  ...
  },
  {
    ...
    description: "Detects workflows with broad top-level permissions where individual jobs do not narrow the scope.",
    category: "permissions",
    severity: "medium",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        if (!isBroad) continue;
        const jobs = getJobs2(workflow.parsed);
        for (const [jobName, job] of jobs) {
          if (!("permissions" in job)) {
            findings.push({
              ...
              description: `Workflow "${workflow.name}" has broad top-level permissions, but job "${jobName}" does not define its own \`permissions:\` block. The job inherits all top-level permissions, which may be more than it needs.`,
              risk: "Each job in a workflow may have different permission requirements. Without job-level overrides, every job gets the broadest permissions declared at the workflow level. A compromise in any job gains access to all granted scopes.",
              remediation: `Add job-level permissions to restrict each job to its minimum required scopes:
...
// src/lib/scanner/data/sensitive-file-patterns.ts
...
// src/lib/scanner/checks/secrets-exposure.ts
...
var secretsExposureChecks = [
  {
    id: "secrets/echoed-to-logs",
    name: "Secrets echoed to logs",
    description: "Detects run blocks that echo or print secrets, which exposes them in workflow logs.",
    ...
    run(context) {
      ...
      for (const workflow of context.workflows) {
        if (!workflow.parsed) continue;
        const jobs = getJobs3(workflow.parsed);
        for (const job of jobs) {
          for (const step of getSteps2(job)) {
            const run2 = step["run"];
            if (typeof run2 !== "string") continue;
            const matches = [
              ...execAll(echoPattern, run2),
              ...execAll(echoPatternQuoted, run2)
            ];
            const seen = /* @__PURE__ */ new Set();
            for (const match of matches) {
              if (match.index !== void 0 && seen.has(match.index)) continue;
              if (match.index !== void 0) seen.add(match.index);
              ...
              findings.push({
                ...
                description: `Workflow "${workflow.name}" contains a run block that prints a secret value to stdout. While GitHub attempts to mask secrets in logs, this masking is not foolproof and can be bypassed through encoding, splitting, or indirect output.`,
                risk: "Secrets written to logs can be viewed by anyone with read access to the repository (for public repos, this means everyone). GitHub's log masking can be bypassed using base64 encoding, character-by-character output, or writing to files that appear in logs. The codecov/codecov-action breach (CVE-2021-27027) demonstrated how CI log exposure leads to credential theft at scale.",
                remediation: 'Never echo secrets directly. Use them only as environment variables or input parameters:\n\n```yaml\nsteps:\n  - run: |\n      # WRONG: echo "${{ secrets.TOKEN }}"\n      # RIGHT: use as environment variable\n      curl -H "Authorization: Bearer $TOKEN" https://api.example.com\n    env:\n      TOKEN: ${{ secrets.TOKEN }}\n```',
                file: workflow.path,
                line: findLineNumber(workflow.content, evidence),
                evidence
              });
  ...
  },
  {
    ...
    description: "Detects secrets interpolated directly into run blocks instead of being passed as environment variables.",
    category: "secrets-exposure",
    severity: "medium",
    run(context) {
      const findings = [];
      ...
      for (const workflow of context.workflows) {
        ...
        for (const job of jobs) {
          for (const step of getSteps2(job)) {
            const run2 = step["run"];
            if (typeof run2 !== "string") continue;
            const matches = execAll(secretsInRun, run2);
            ...
            for (const match of matches) {
              ...
              findings.push({
                ...
                description: `Workflow "${workflow.name}" uses a secret expression directly in a \`run:\` block. This exposes the secret value in the process command line, which may appear in process listings, shell history, or error messages.`,
                risk: "Secrets passed as command-line arguments appear in /proc on Linux runners and can be captured by other processes. If the command fails, error output may include the secret value. This also makes the workflow vulnerable to injection if the secret contains shell metacharacters.",
                remediation: 'Pass secrets through environment variables instead of inline interpolation:\n\n```yaml\nsteps:\n  - run: curl -H "Authorization: Bearer $TOKEN" https://api.example.com\n    env:\n      TOKEN: ${{ secrets.API_TOKEN }}\n```\n\nEnvironment variables are not visible in process listings and are the recommended way to pass secrets to shell commands.',
                file: workflow.path,
                line: findLineNumber(workflow.content, evidence),
                evidence
              });
  ...
  },
  {
    ...
    description: "Detects actions/checkout steps that leave Git credentials persisted in .git/config.",
    category: "secrets-exposure",
    severity: "medium",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        if (checkoutSteps.length > 0) {
          findings.push({
            ...
            description: `Workflow "${workflow.name}" has ${checkoutSteps.length} checkout step${checkoutSteps.length === 1 ? "" : "s"} without \`persist-credentials: false\`. By default, the checkout action stores the GITHUB_TOKEN in the local .git/config, making it available to all subsequent steps.`,
            risk: "Any subsequent step (including third-party actions) can read the persisted token from .git/config. If a later action is compromised, it can use this token to push code, create releases, or access the GitHub API with the workflow's permissions.",
            remediation: "Set `persist-credentials: false` on checkout steps:\n\n```yaml\nsteps:\n  - uses: actions/checkout@v4\n    with:\n      persist-credentials: false\n```\n\nWhen you need to push changes, explicitly configure Git credentials for only the step that needs them.",
            file: workflow.path,
...
${checkoutSteps.join("\n")}`
  ...
  },
  {
    ...
    description: "Detects upload-artifact steps whose path includes potentially sensitive file patterns.",
    category: "secrets-exposure",
    severity: "medium",
    run(context) {
      const findings = [];
        ...
        for (const job of jobs) {
          for (const step of getSteps2(job)) {
            ...
            for (const pattern of SENSITIVE_FILE_PATTERNS) {
              ...
              if (artifactPath.includes(pattern) || plainPattern && artifactPath.includes(plainPattern)) {
                ...
                findings.push({
                  ...
                  description: `Workflow "${workflow.name}" uploads an artifact with path "${artifactPath}" which matches sensitive file pattern "${pattern}". This may inadvertently include credentials, private keys, or configuration files with secrets.`,
                  risk: "Workflow artifacts are downloadable by anyone with read access to the repository. Accidentally uploading credential files, private keys, or environment files exposes secrets to unauthorized parties. Artifacts persist for the configured retention period (default 90 days).",
                  remediation: "Review the artifact path and exclude sensitive files:\n\n```yaml\nsteps:\n  - uses: actions/upload-artifact@v4\n    with:\n      name: build-output\n      path: |\n        dist/\n        !dist/**/*.env\n        !dist/**/*.key\n        !dist/**/*.pem\n```\n\nUse the exclusion pattern (`!`) to filter out sensitive files from artifact uploads.",
                  file: workflow.path,
...
// src/lib/scanner/checks/runner-security.ts
...
var runnerSecurityChecks = [
  {
    id: "runner/self-hosted-pr",
    name: "Self-hosted runner on pull_request",
    description: "Detects self-hosted runners used with pull_request triggers, allowing fork PRs to execute code on your infrastructure.",
    ...
    run(context) {
      ...
      for (const workflow of context.workflows) {
        if (!workflow.parsed) continue;
        const triggers = getTriggers2(workflow.parsed);
        if (!triggers.includes("pull_request")) continue;
        const jobs = getJobs4(workflow.parsed);
        for (const [jobName, job] of jobs) {
...
# gha-scanner-ignore: runner/self-hosted-pr`,
  ...
  },
  {
    ...
    description: "Detects self-hosted runners used with triggers that accept external input.",
    category: "runner-security",
    severity: "high",
    run(context) {
      const findings = [];
      ...
      for (const workflow of context.workflows) {
        ...
        for (const [jobName, job] of jobs) {
          ...
          findings.push({
            ...
            description: `Workflow "${workflow.name}" runs job "${jobName}" on a self-hosted runner with triggers that accept external input: ${matchingTriggers.join(", ")}. These triggers can be activated by external contributors or users.`,
            risk: "Self-hosted runners persist state between jobs. Triggers like issue_comment and discussion_comment allow any GitHub user to invoke workflows. Combined with a self-hosted runner, this can allow unauthorized code execution on internal infrastructure, access to cached secrets and credentials, and lateral movement within your network.",
            remediation: `Move untrusted-trigger workflows to GitHub-hosted runners:
...
If you must use self-hosted runners, use runner groups with repository access policies, and always run self-hosted runners in ephemeral (disposable) mode with \`--ephemeral\` flag.`,
            file: workflow.path,
            line: findLineNumber(workflow.content, "self-hosted"),
            evidence
          });
  ...
  },
  {
    ...
    description: "Detects run blocks that execute Docker with privileged mode or Docker socket mounting.",
    category: "runner-security",
    severity: "high",
    run(context) {
      const findings = [];
      ...
      for (const workflow of context.workflows) {
        ...
        const jobs = getJobs4(workflow.parsed);
        for (const [, job] of jobs) {
          for (const step of getSteps3(job)) {
            const run2 = step["run"];
            if (typeof run2 !== "string") continue;
            const privilegedMatches = execAll2(privilegedPattern, run2);
            for (const match of privilegedMatches) {
              ...
              findings.push({
                ...
                description: `Workflow "${workflow.name}" runs a Docker container with \`--privileged\`, disabling all security isolation between the container and the host.`,
                risk: "The --privileged flag gives the container full access to the host system, including all devices, capabilities, and kernel features. An attacker who compromises the workflow can escape the container and access the host runner, other containers, and any secrets or credentials on the machine.",
                remediation: "Remove the `--privileged` flag and use specific capabilities instead:\n\n```yaml\n- run: |\n    docker run --cap-add SYS_PTRACE my-image  # only add needed caps\n```\n\nIf privileged access is truly needed, run this on an ephemeral GitHub-hosted runner to limit blast radius.",
                file: workflow.path,
            ...
            const socketMatches = execAll2(socketPatternAlt, run2);
            for (const match of socketMatches) {
              ...
              findings.push({
                ...
                description: `Workflow "${workflow.name}" mounts the Docker socket (\`/var/run/docker.sock\`) into a container, granting full control over the host's Docker daemon.`,
                risk: "Mounting the Docker socket is equivalent to granting root access to the host. A compromised container can spawn new privileged containers, access any volume on the host, and execute arbitrary commands as root. This is a well-known container escape technique.",
                remediation: "Avoid mounting the Docker socket. Use alternatives:\n\n```yaml\n- run: |\n    # Use buildx for building images without socket access\n    docker buildx build --push -t my-image .\n```\n\nIf Docker-in-Docker is required, use the `dind` (Docker-in-Docker) service approach with TLS.",
                file: workflow.path,
...
// src/lib/scanner/checks/ci-cd-hygiene.ts
function getJobs5(parsed) {
  const jobs = parsed["jobs"];
  if (!jobs || typeof jobs !== "object") return [];
  return Object.entries(jobs).filter(
    (entry) => typeof entry[1] === "object" && entry[1] !== null
...
var ciCdHygieneChecks = [
  {
    id: "hygiene/no-concurrency",
    name: "No concurrency controls",
    description: "Checks whether workflows define concurrency groups to prevent duplicate or conflicting runs.",
    ...
    run(context) {
      ...
      for (const workflow of context.workflows) {
        ...
        const jobs = getJobs5(workflow.parsed);
        ...
        findings.push({
          ...
          description: `Workflow "${workflow.name}" does not define a \`concurrency:\` group at the workflow or job level. Without concurrency controls, multiple instances of the same workflow can run simultaneously.`,
          risk: "Concurrent workflow runs can cause race conditions in deployments, duplicate notifications, wasted compute costs, and conflicts when multiple runs modify the same resources. For deployment workflows, this can lead to inconsistent states.",
          remediation: "Add a concurrency group to prevent duplicate runs:\n\n```yaml\nconcurrency:\n  group: ${{ github.workflow }}-${{ github.ref }}\n  cancel-in-progress: true  # cancel older runs for the same branch\n```\n\nFor deployment workflows, omit `cancel-in-progress` to ensure deployments complete.",
          file: workflow.path,
          line: 1,
  ...
  },
  {
    ...
    description: "Checks whether jobs define timeout-minutes to prevent runaway builds.",
    category: "ci-cd-hygiene",
    severity: "info",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        if (!allJobsHaveTimeout && jobs.length > 0) {
          const jobsWithout = jobs.filter(([, job]) => !("timeout-minutes" in job)).map(([name]) => name);
          findings.push({
            checkId: "hygiene/no-timeout",
            severity: "info",
            category: "ci-cd-hygiene",
            title: "Jobs without timeout-minutes",
            description: `Workflow "${workflow.name}" has jobs without \`timeout-minutes\`: ${jobsWithout.join(", ")}. The default timeout is 6 hours (360 minutes), which can waste runner capacity if a job hangs.`,
            risk: "Without explicit timeouts, a hung job (e.g., waiting on a network resource, stuck in an infinite loop, or a cryptominer injected via a compromised dependency) will consume runner minutes for up to 6 hours. For self-hosted runners, this can block other jobs. Attackers have exploited long-running CI jobs for cryptocurrency mining.",
            remediation: "Set appropriate timeouts on all jobs:\n\n```yaml\njobs:\n  build:\n    timeout-minutes: 15  # adjust based on expected duration\n    runs-on: ubuntu-latest\n    steps:\n      - ...\n```",
            file: workflow.path,
  ...
  },
  {
    ...
    description: "Detects steps with continue-on-error: true, which can mask failures including security check failures.",
    category: "ci-cd-hygiene",
    severity: "medium",
    run(context) {
      const findings = [];
      for (const workflow of context.workflows) {
        ...
        const jobs = getJobs5(workflow.parsed);
        for (const [jobName, job] of jobs) {
          for (const step of getSteps4(job)) {
            ...
            findings.push({
              ...
              description: `Workflow "${workflow.name}" has a step with \`continue-on-error: true\` in job "${jobName}". This causes the job to continue even if this step fails, potentially masking important failures.`,
              risk: "Using continue-on-error can silently swallow failures from security scanning steps, linting, tests, or validation. An attacker who compromises a dependency may cause a security check to fail, but the pipeline will continue to deploy regardless. This is especially dangerous on steps that run security tools, sign artifacts, or validate inputs.",
              remediation: "Remove `continue-on-error: true` or use conditional steps instead:\n\n```yaml\nsteps:\n  - id: security-scan\n    run: trivy fs .\n    # Do NOT use continue-on-error for security checks\n\n  # If you need optional steps, use conditionals:\n  - run: optional-step\n    if: always()  # runs regardless, but failure still reported\n```",
              file: workflow.path,
...
// src/lib/scanner/checks/best-practices.ts
var bestPracticesChecks = [
  {
    id: "practices/no-dependabot",
    name: "No Dependabot configuration for GitHub Actions",
    description: "Checks whether the repository has Dependabot configured to keep GitHub Actions up to date.",
    ...
    run(context) {
      ...
      if (!context.hasDependabot) {
        findings.push({
          ...
          description: "This repository does not have a `.github/dependabot.yml` configuration file. Dependabot automatically creates pull requests to keep dependencies (including GitHub Actions) up to date with security patches.",
          risk: "Without automated dependency updates, GitHub Actions used in workflows may fall behind on security patches. The tj-actions/changed-files supply chain attack (CVE-2025-30066) and the Aqua Trivy compromise (March 2026) both demonstrated that outdated or unpinned actions are primary targets. Dependabot helps ensure you are notified of and can quickly adopt security fixes.",
          remediation: 'Create `.github/dependabot.yml` with at minimum a github-actions ecosystem entry:\n\n```yaml\nversion: 2\nupdates:\n  - package-ecosystem: "github-actions"\n    directory: "/"\n    schedule:\n      interval: "weekly"\n```\n\nAlso consider adding entries for your other package ecosystems (npm, pip, etc.).',
      ...
      if (context.dependabotConfig) {
        ...
        if (Array.isArray(updates)) {
          const hasActionsEcosystem = updates.some(
            (entry) => typeof entry === "object" && entry !== null && entry["package-ecosystem"] === "github-actions"
          );
          if (!hasActionsEcosystem) {
            findings.push({
              ...
              description: "The repository has a Dependabot configuration, but it does not include an entry for the `github-actions` package ecosystem. GitHub Actions dependencies will not receive automated update PRs.",
              risk: "While other dependencies may be kept current, GitHub Actions used in workflows will not receive automated security updates. Actions are a common supply chain attack vector, and keeping them updated is critical for catching compromised versions quickly.",
              remediation: 'Add a github-actions entry to your existing `.github/dependabot.yml`:\n\n```yaml\nupdates:\n  # ... existing entries ...\n  - package-ecosystem: "github-actions"\n    directory: "/"\n    schedule:\n      interval: "weekly"\n```',
              file: ".github/dependabot.yml",
  ...
  },
  {
    ...
    description: "Checks whether the repository has CODEOWNERS rules protecting workflow files from unauthorized changes.",
    category: "best-practices",
    severity: "low",
    run(context) {
      const findings = [];
      if (!context.hasCodeowners) {
        findings.push({
          ...
          description: "This repository does not have a CODEOWNERS file. CODEOWNERS enables mandatory code review by designated owners for changes to critical paths like workflow files.",
          risk: "Without CODEOWNERS, any contributor with write access can modify workflow files without mandatory review from security-aware team members. Workflow modifications are a high-impact change since they control CI/CD pipelines, have access to secrets, and can modify deployment processes.",
          remediation: 'Create a CODEOWNERS file (in `.github/CODEOWNERS`, `CODEOWNERS`, or `docs/CODEOWNERS`) with workflow protection:\n\n```\n# Require security team review for workflow changes\n.github/workflows/ @org/security-team\n.github/actions/   @org/security-team\n```\n\nEnable the "Require review from Code Owners" branch protection rule to enforce these reviews.',
          file: "CODEOWNERS",
      ...
      if (context.codeownersContent) {
        ...
        const hasWorkflowRule = lines.some((line) => {
          ...
          return trimmed.includes(".github/workflows") || trimmed.includes(".github/workflows/") || trimmed.includes(".github/");
        });
        if (!hasWorkflowRule) {
          findings.push({
            checkId: "practices/no-codeowners",
            ...
            description: "The repository has a CODEOWNERS file, but it does not include a rule for `.github/workflows/`. Workflow files can be modified without mandatory review from designated code owners.",
            risk: "Workflow files are high-value targets for supply chain attacks. Without CODEOWNERS protection, a compromised contributor account or a social engineering attack can modify workflow files through PRs that may not receive sufficient security review.",
            remediation: "Add a workflow protection rule to your CODEOWNERS file:\n\n```\n# Require security team review for CI/CD changes\n.github/workflows/ @org/security-team\n.github/actions/   @org/security-team\n.github/dependabot.yml @org/security-team\n```",
            file: "CODEOWNERS",
...
// src/lib/scanner/checks/index.ts
var allChecks = [
  ...supplyChainChecks,
  ...injectionChecks,
...
var MAX_FINDINGS_PER_CHECK = 3;
function calculateScore(findings, checks) {
  ...
  const findingsByCheck = /* @__PURE__ */ new Map();
...
function runScan(context) {
  ...
  const findings = [];
  const warnings = [...context.parseWarnings];
  let passingChecks = 0;
...
var VALID_FAIL_ON = ["critical", "high", "medium", "low", "info", "none"];
